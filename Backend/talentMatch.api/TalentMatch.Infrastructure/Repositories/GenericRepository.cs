...
using TalentMatch.Infrastructure.Extensions;
...
namespace TalentMatch.Infrastructure.Repositories
{
    public class GenericRepository<TEntity> : IGenericRepository<TEntity> where TEntity : class
    {
        ...
        public virtual async Task<IEnumerable<TEntity>> GetPagedElementsAsync<k>(
            ...
            int pageIndex,
            int pageCount,
            Expression<Func<TEntity, k>> orderByExpression = null,
            bool ascending = true,
            string includeProperties = ""
            )
        {
            return await Task.FromResult(
                                 GetPagedElements(filter, pageIndex, pageCount, orderByExpression, ascending, includeProperties))
                             .ConfigureAwait(false);
        }
